...
export default function InstructorCoursesSidebar({
  instructorName = "Giảng viên",
  onCollapse,
}: InstructorCoursesSidebarProps) {
  ...
  const navItems = [
    ...
    },
    {
      ...
      icon: Users,
      href: "/courses-instructor/student-manage",
    },
    {
      label: "Hỏi và đáp",
      icon: HelpCircle,
      href: "/courses-instructor/qna",
    },
    {
      label: "Thống kê khóa học",
      icon: BarChart3,
      href: "/courses-instructor/analytics",
    },
    {
      label: "Hồ sơ giảng viên",
      icon: UserCircle2,
      href: "/courses-instructor/profile",
    },
  ];
      ...
      <div className="flex flex-col h-full p-3 md:p-4">
        ...
        <div className="border-t border-slate-800 mt-4 pt-4 space-y-3">
          ...
          >
            ...
            {!collapsed && (
              <div className="flex flex-col justify-start transition-transform duration-200 group-hover:translate-x-0.5">
                <p className="text-sm font-semibold truncate">
                  {instructorName}
                </p>
                <p className="text-xs flex text-slate-400">Giảng viên</p>
              </div>
            )}
          </button>
